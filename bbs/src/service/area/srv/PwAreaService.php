 ...
 * @version $Id: PwAreaService.php 21673 2012-12-12 07:28:25Z gao.wanggao $
...
class PwAreaService {
	...
	public function fetchAreaInfo($areaids) {
		if (!$list = $this->_getWindidAreaDs()->fetchArea($areaids)) {
			return array();
		}
		$array = array();
	...
	public function fetchAreaRout($areaids) {
		$list = $this->_getWindidAreaDs()->fetchArea($areaids);
		...
		$list = $this->_getWindidAreaDs()->fetchArea(array_keys($parents));
		foreach ($list as $key => $_item) {
	...
	public function getAreaTree() {
		$areas = $this->_getWindidAreaDs()->getAll();
		$root = array();
