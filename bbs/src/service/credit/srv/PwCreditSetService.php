 ...
 * @version $Id: PwCreditSetService.php 21452 2012-12-07 10:18:33Z gao.wanggao $
...
class PwCreditSetService {
	...
	public function getCredit() {
		return $this->_getWindid()->getConfig('credit:credits');
	}
	...
	public function setCredits($credits, $newCredit = array()) {
		...
		//更新windid的积分设置
		$this->_getWindid()->setConfig('credit','credits', $credits);
	...
	public function deleteCredit($creditId) {
		...
		$credits = $windid->getConfig('credit:credits');
		unset($credits[$creditId]);
		$windid->deleteConfig('credit', $data);
		$windid->setConfig('credit','credits', $credits);
