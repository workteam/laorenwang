...
class ModelInstall extends Model {
	public function mysql($data) {
		...
		$file = DIR_APPLICATION . 'hczm.cn.sql';
		...
		if ($sql = file($file)) {
			...
			mysql_query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `group` = 'config', `key` = 'config_url', value = '" . mysql_real_escape_string(HTTP_hczm.cn) . "'", $connection);
